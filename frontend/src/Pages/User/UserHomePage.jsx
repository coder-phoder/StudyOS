    ...
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-[1180px] px-5 pb-24 pt-5">
        ...
        <div className="mb-[14px] flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="m-0 text-[24px] font-bold">Dashboard</h1>
            <p className="mt-1 text-[13px] text-slate-400">Your StudyOS profile and workspace overview.</p>
          </div>
          <button
            type="button"
            onClick={() => void handleRefresh()}
            disabled={isRefreshing}
            className="inline-flex items-center justify-center rounded-[10px] border border-white/[0.09] bg-white/[0.045] px-[15px] py-[9px] text-[13px] font-semibold text-slate-100 transition hover:border-cyan-400/40 hover:bg-white/[0.075] focus:outline-none focus:ring-2 focus:ring-cyan-400/40 disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        {actionError && (
          <div role="alert" className="mb-[14px] rounded-xl border border-rose-400/35 bg-rose-400/10 px-4 py-3 text-sm text-rose-100">
            {actionError}
          </div>
        )}
        ...
        <section className="relative overflow-hidden rounded-[18px] border border-white/[0.09] bg-white/[0.045] p-5 shadow-[0_8px_30px_-14px_rgba(0,0,0,0.6)] before:pointer-events-none before:absolute before:inset-0 before:bg-linear-to-br before:from-white/[0.05] before:to-transparent before:content-['']">
          <div className="relative">
              ...
              <div className="flex items-center gap-4">
                {showAvatar ? (
                  <img
                    ...
                    className="h-20 w-20 rounded-[14px] border border-cyan-400/35 object-cover"
                  />
                ) : (
                  <div className="grid h-20 w-20 place-items-center rounded-[14px] border border-cyan-400/35 bg-violet-400/15 [font-family:var(--font-display)] text-2xl font-bold text-cyan-400">
                    {initialsFor(profile.username)}
                  </div>
                )}
                <div>
                  <h2 className="text-xl font-bold">{profile.username || 'StudyOS member'}</h2>
                  <p className="mt-1 text-sm text-slate-400">{profile.email || 'No email address available'}</p>
                </div>
              </div>
              <span className={`w-fit rounded-full px-3 py-1 text-[11px] font-bold ${profile.isActive !== false ? 'bg-emerald-400/15 text-emerald-300' : 'bg-rose-400/15 text-rose-200'}`}>
            ...
            <div className="mt-[14px] rounded-[12px] border border-white/[0.09] bg-black/15 p-4">
              <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">Bio</p>
...
function ProfileItem({ label, value, mono = false }) {
  return (
    <div className="rounded-[12px] border border-white/[0.09] bg-white/[0.03] p-[14px]">
      <p className="text-[10.5px] font-semibold uppercase tracking-[0.07em] text-slate-400">{label}</p>
      <p className={`mt-2 break-words text-[13px] text-slate-200 ${mono ? 'font-mono text-[11px]' : ''}`}>{value || 'Not available'}</p>
    </div>
  )
}
